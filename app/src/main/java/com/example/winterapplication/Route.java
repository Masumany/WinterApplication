...
public class Route extends AppCompatActivity {
    ...
    List<RouteNew.Data> data = new ArrayList<>();
    private TextView BackTextView;
    private FloatingActionButton returnTop;
    List<RouteNew.Data> filteredList = new ArrayList<>();
    ...
    private void initClick() {
        if (BackTextView!= null) {
            BackTextView.setOnClickListener(v -> {
                Intent intent = new Intent(Route.this, MainActivity.class);
                // 启动该意图，实现页面跳转到首页面
    ...
    private void fetchDataFromApi() {
        ...
        Request request = new Request.Builder()
                .url("https://wanandroid.com/popular/route/json")
                .get()
        ...
        okHttpClient.newCall(request).enqueue(new Callback() { // 将请求加入请求队列，使用 enqueue 方法异步执行请求，并添加一个回调函数，当请求成功时调用 onResponse 方法，失败时调用 onFailure 方法
            ...
            public void onFailure(Call call, IOException e) {
                Log.e("Route", "网络请求失败: " + e.getMessage());
            }
        });
    ...
    private void handleResponse(String result) {
        Gson gson = new Gson();
        RouteNew response = gson.fromJson(result, RouteNew.class);
